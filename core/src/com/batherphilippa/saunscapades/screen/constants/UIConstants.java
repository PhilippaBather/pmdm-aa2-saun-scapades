package com.batherphilippa.saunscapades.screen.constants;

/**
 * UIConstants - los constantes de la interfaz del usuario (UI).
 */
public class UIConstants {

    public static final String BTN_CONFIG = "CONFIGURATIONS";
    public static final String BTN_EXIT = "EXIT";
    public static final String BTN_MAIN_MENU = "MAIN MENU";
    public static final String BTN_GAME_MENU = "MENU";
    public static final String BTN_PLAY = "PLAY";
    public static final String BTN_PLAY_AGAIN = "PLAY AGAIN";
    public static final String BTN_TOP_TEN = "TOP TEN";
    public static final String BTN_RESUME = "Resume Game";
    public static final String BTN_SAVE_SCORE = "SAVE SCORE";

    // opciones de configuración
    public static final String LABEL_GAME_LENGTH = "Select Game Length";
    public static final String LABEL_GAME_DIFFICULTY = "Select game difficulty";
    public static final String LABEL_SOUND_ON_OFF = "Sound On/Off";

    // opciones de entrada
    public static final String TEXT_FIELD_INPUT_NAME = "  Enter your name";

    // títulos y etiquetas de pantallas
    public static final String LABEL_GAME_FINISHED = "!!! Congratulations: Shaun Scapades Conquered!!!";
    public static final String LABEL_GAME_OVER_TITLE = "GAME OVER!!!";
    public static final String LABEL_GAME_SCREEN_MENU = "Game Screen Menu";
    public static final String LABEL_MAIN_MENU_TITLE = "MAIN MENU";
    public static final String LABEL_OPTIONS_MENU_TITLE = "SELECT CONFIGURATION OPTIONS";
    public static final String LABEL_TOP_TEN_SCORES = "Top 10 Scores";
    public static final String LABEL_BETTER_LUCK_NEXT_GAME = "Better luck next time  - you need %d to get on the Top Ten Scores list.";

    // información del primer nivel
    public static final String LABEL_LEVEL_1_INFO_TITLE = "Level 1 Mission";
    public static final String LABEL_LEVEL_1_INFO_1 = "Navigate across the countryside to find the balloons.";
    public static final String LABEL_LEVEL_1_INFO_2 = "Avoid the angry sheep: sap your energy they will.";
    public static final String LABEL_LEVEL_1_INFO_3 = "Collect gold coins - every sheep needs them.";
    public static final String LABEL_LEVEL_1_INFO_4 = "Fall in the sea at your peril.";
    public static final String LABEL_LEVEL_1_INFO_5 = "Beware of the bombs for they will take your life.";

    // información del segundo nivel
    public static final String LABEL_MISSION_2 = "Mission 2";
    public static final String LABEL_LEVEL_2_INFO_1 = "Navigate across the wasteland to find the balloons.";
    public static final String LABEL_LEVEL_2_INFO_2 = "Save the trapped Shirley sheep from the bombs.";
    public static final String LABEL_LEVEL_2_INFO_3 = "Failure to do so will incur their death and - rightly so - loss of points.";
    public static final String LABEL_LEVEL_2_INFO_4 = "Avoid the Kamikaze sheep for they will temporarily paralyse you.";
    public static final String TEXT_FIELD_LEVEL_END = "Congratulations: Level Conquered!";
    public static final String TEXT_FIELD_SCORE = "SCORE: %s";
}
